...
router.post("/matches", (req, res) => {
  ...
  generateScoreMatrix(
    { patient: patientJson, level: "childMatches", type: "parent" },
    () => {
  ...
  function generateScoreMatrix({ patient, level, type }, callback) {
    ...
    matchingTool.performMatch(
      ...
      ({
        ...
        let link =
          patient.link && patient.link.length > 0
            ? patient.link[0].other.reference
            : null;
        let goldenLink = null;
        ...
        if (
          entry.resource &&
          entry.resource !== null &&
          entry.resource.address.length > 0
        ) {
        ...
        if (
          ...
          village =
            entry.resource.address[0]?.extension
              .find((ext) => ext.url === "http://fhir.openmrs.org/ext/address")
              .extension.find(
                (ext) =>
                  ext.url === "http://fhir.openmrs.org/ext/address#village"
              ).valueString ?? "Not Set";
        }
        ...
        if (
          ...
          parish =
            entry.resource.address[0]?.extension
              ...
              .extension.find(
                (ext) =>
                  ext.url === "http://fhir.openmrs.org/ext/address#parish"
              ).valueString ?? "Not Set";
        }
        ...
        if (
          ...
          sub_county =
            entry.resource.address[0]?.extension
              ...
              .extension.find(
                (ext) =>
                  ext.url === "http://fhir.openmrs.org/ext/address#subcounty"
              ).valueString ?? "Not Set";
        }
        ...
        let primaryPatient = {
          ...
          ouid: goldenLink,
          source_id:
            validSystem && validSystem.value ? validSystem.value : null,
  ...
  if (query && !dontSaveChanges) {
    fhirWrapper.getResource(
      ...
      (resourceData) => {
        logger.error(
          "Processing Unbreak " +
            JSON.stringify(resourceData.entry, 0, 2) +
            " patients"
        );
        const auditedId = [];
        for (const idPair of ids) {
...
router.get(`/cr-stats`, (req, res) => {
  ...
  const allMatchIssuesURL = `${FHIR_BASE_URL}/Patient?_tag=${matchIssuesURI}|potentialMatches,${matchIssuesURI}|conflictMatches&_summary=count`;

  // Define a helper function to wrap the `request` call in a Promise
  const fetchData = (url) => {
  ...
  Promise.all([
    ...
    .then(
      ([
        ...
        const conflictMatches = conflictData.total || 0;
        const potentialMatches = potentialData.total || 0;
        const totalMatches = matchesData.total || 0;
        const autoMatches = autoData.total || 0;
    ...
    .catch((error) => {
      ...
      res.status(500).json({
        error: "Failed to fetch patient stats. Please try again later.",
      });
    });
});
